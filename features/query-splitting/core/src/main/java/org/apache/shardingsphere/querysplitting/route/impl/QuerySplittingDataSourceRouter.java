...
package org.apache.shardingsphere.querysplitting.route.impl;
...
public final class QuerySplittingDataSourceRouter {
    
    private final QuerySplittingDataSourceRule rule;
    ...
    public String route(final SQLStatementContext<?> sqlStatementContext) {
        ...
        if ((1 == sqlStatementContext.getTablesContext().getTables().size() && !containsGroupBySegment(sqlStatementContext.getSqlStatement()))
                || 0 == sqlStatementContext.getTablesContext().getTables().size()) {
    ...
    private boolean containsGroupBySegment(final SQLStatement sqlStatement) {
        return sqlStatement instanceof SelectStatement && SelectStatementHandler.getGroupBySegment((SelectStatement) sqlStatement).isPresent();
    }
    ...
    private boolean isAllowWriteDataSourceQuery() {
        return rule.getEnabledReplicaDataSources().isEmpty() && (rule.getQuerySplittingStrategy() instanceof DynamicQuerySplittingStrategy)
                && ((DynamicQuerySplittingStrategy) rule.getQuerySplittingStrategy()).isAllowWriteDataSourceQuery();
    }
}
