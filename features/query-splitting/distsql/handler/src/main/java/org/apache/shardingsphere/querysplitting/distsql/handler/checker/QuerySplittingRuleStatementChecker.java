...
package org.apache.shardingsphere.querysplitting.distsql.handler.checker;
 ...
 */
public final class QuerySplittingRuleStatementChecker {
     ...
     */
    public static void checkCreation(final ShardingSphereDatabase database, final Collection<QuerySplittingRuleSegment> segments,
                                     final QuerySplittingRuleConfiguration currentRuleConfig, final boolean ifNotExists) {
     ...
     * @param currentRuleConfig current rule config
     */
    public static void checkAlteration(final ShardingSphereDatabase database, final Collection<QuerySplittingRuleSegment> segments, final QuerySplittingRuleConfiguration currentRuleConfig) {
        String databaseName = database.getName();
     ...
     * @param currentRuleConfig current rule config
     */
    public static void checkRuleConfigurationExist(final ShardingSphereDatabase database, final QuerySplittingRuleConfiguration currentRuleConfig) {
    ...
    private static void checkRuleNamesExist(final Collection<QuerySplittingRuleSegment> segments, final QuerySplittingRuleConfiguration currentRuleConfig, final String databaseName) {
        Collection<String> requiredRuleNames = segments.stream().map(QuerySplittingRuleSegment::getName).collect(Collectors.toList());
    ...
    private static void checkDuplicateRuleNames(final String databaseName, final Collection<QuerySplittingRuleSegment> segments, final QuerySplittingRuleConfiguration currentRuleConfig,
    ...
    private static void checkDuplicateRuleNamesWithSelf(final String databaseName, final Collection<QuerySplittingRuleSegment> segments) {
        Collection<String> duplicatedRuleNames = getDuplicated(segments.stream().map(QuerySplittingRuleSegment::getName).collect(Collectors.toList()));
    ...
    private static void checkDuplicateRuleNamesWithResourceMetaData(final ShardingSphereResourceMetaData resourceMetaData, final Collection<QuerySplittingRuleSegment> segments) {
        ...
        Collection<String> toBeCreatedRuleNames = segments.stream().map(QuerySplittingRuleSegment::getName).filter(currentRuleNames::contains).collect(Collectors.toList());
    ...
    private static void checkDuplicateRuleNamesWithRuleConfiguration(final String databaseName, final QuerySplittingRuleConfiguration currentRuleConfig,
                                                                     final Collection<QuerySplittingRuleSegment> segments) {
        Collection<String> currentRuleNames = new LinkedList<>();
    ...
    private static void checkDataSourcesExist(final String databaseName, final Collection<QuerySplittingRuleSegment> segments, final ShardingSphereDatabase database) {
    ...
    private static void checkDuplicatedDataSourceNames(final String databaseName, final Collection<QuerySplittingRuleSegment> segments,
                                                       final QuerySplittingRuleConfiguration currentRuleConfig, final boolean isCreating) {
        ...
        if (null != currentRuleConfig) {
            ...
            currentRuleConfig.getDataSources().forEach(each -> {
                if (null != each.getStaticStrategy() && !toBeAlteredRuleNames.contains(each.getName())) {
                    existedWriteDataSourceNames.add(each.getStaticStrategy().getMysqlDataSourceName());
                    existedReadDataSourceNames.addAll(each.getStaticStrategy().getSnowDataSourceNames());
                }
            });
        }
    ...
    private static Collection<String> getToBeAlteredRuleNames(final Collection<QuerySplittingRuleSegment> segments) {
        return segments.stream().map(QuerySplittingRuleSegment::getName).collect(Collectors.toSet());
    }
    
    private static void checkDuplicateWriteDataSourceNames(final String databaseName, final Collection<QuerySplittingRuleSegment> segments, final Collection<String> writeDataSourceNames) {
        segments.forEach(each -> {
    ...
    private static void checkDuplicateReadDataSourceNames(final String databaseName, final Collection<QuerySplittingRuleSegment> segments, final Collection<String> readDataSourceNames) {
        segments.forEach(each -> {
    ...
    private static void checkLoadBalancers(final Collection<QuerySplittingRuleSegment> segments) {
        Collection<String> notExistedLoadBalancers = segments.stream().map(QuerySplittingRuleSegment::getLoadBalancer).filter(Objects::nonNull).distinct()
